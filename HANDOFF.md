...
**Дополнено (этот же день, `da0c04b`+`bb0db6d`) — ОБА в `main`, тоже ещё не задеплоено:**
- ✅ **Игра на русском** + выбор числа раундов (1..8) + **Фаза 5: реальные `points` за победы**
  (realtime пишет `users.points` прямым pg UPDATE по итогам) + финал-экран `GameFinale`
  (подиум, таблица итогов с `+pts`, «трибуна»). Маскировка слов теперь понимает кириллицу.
  **`realtime/game.mjs` менялся → контейнер realtime обязательно пересобрать.**
- ✅ **Глубокая кастомизация аватара** (`bb0db6d`): `Avatar.tsx` переписан в **псевдо-3D**
  (градиенты/блик/обводка/свечение), новые категории `head`/`antenna`/`effect` (анимация —
  CSS в `globals.css`), глаза/рты ×8. Премиум eyes:3/mouth:3 → free (фикс «4-й рот»). Редактор —
  живое превью каждой опции. **Миграций НЕ требует** (аватар — в JSON-колонке `avatar`, очки — в
  существующей `points`). Обратная совместимость: опции только дописаны в конец массивов.

**Следующее по плану (роадмап ниже):** Фаза 5 закрыта (очки за игру начисляются). Идеи на потом:
очки за угадывание-как-валюта уже работают на разблокировки аватара; daily-загадка; модерация игры.

---

## Координаты

- **Домен:** `bash-app.com` (DNS — Cloudflare; A-запись `@` и `www` должны быть **DNS only / серое облако** для certbot).
  Прежний `klebold.xyz` потерян (регистратор заблокировал аккаунт). Миграция домена — см. блок «Смена домена» ниже.
...
- **Почта:** Resend. From: `bash-app.com <noreply@send.bash-app.com>`. **Новый sending-домен `send.bash-app.com`
  нужно заново верифицировать в Resend** (добавить DKIM/SPF в DNS bash-app.com). API-ключ Resend прежний.
- **Spotify:** приложение в **dev-mode** (макс 25 whitelisted аккаунтов в User Management).

## Смена домена `klebold.xyz` → `bash-app.com` (2026-07-01)
Регистратор заблокировал аккаунт и снял доступ к `klebold.xyz`. Куплен новый `bash-app.com`, DNS (Cloudflare)
уже указывает A `@`/`www` на VPS. VPS, БД (`pgdata`), контейнеры — целы; менялся только «домен-слой».
**В коде домен уже заменён** (коммит про смену домена): титул, CLI-промпт/neofetch, boot, публичный профиль
(`/u/*` мета + шэр-ссылка теперь через `window.location.origin`), `.env.example`, `SITE_NAME`. Осталось (руками):
1. **`.env` на VPS** (`nano ~/terminal-site/.env`): `AUTH_URL="https://bash-app.com"`,
   `EMAIL_FROM="bash-app.com <noreply@send.bash-app.com>"`, `SPOTIFY_REDIRECT_URI="https://bash-app.com/api/spotify/callback"`,
   `SITE_NAME="bash-app.com"`. Остальное (секреты, DATABASE_URL) НЕ трогать.
2. **Resend:** добавить и верифицировать sending-домен `send.bash-app.com` (DKIM/SPF в DNS). API-ключ прежний.
3. **Spotify dashboard:** в Redirect URIs добавить `https://bash-app.com/api/spotify/callback` (точное совпадение с `.env`).
4. **nginx:** `server_name bash-app.com www.bash-app.com;` → `certbot --nginx -d bash-app.com -d www.bash-app.com`.
5. **Деплой:** `git pull && docker compose up -d --build`. Проверка: HTTPS, регистрация/письмо, чат, Spotify.
...
`POSTGRES_USER/PASSWORD/DB`, `DATABASE_URL` (host=`db`), `AUTH_SECRET`, `AUTH_URL=https://bash-app.com`,
...
5. ✅ ~~**Фаза 5 — очки и разблокировки**~~ — СДЕЛАНО (`da0c04b`): по концу партии realtime
   начисляет реальные `points` прямым `pg` UPDATE (база за счёт + бонус топ-3); они открывают
   locked-опции аватара (`lib/avatar.ts`, `cost`). Аватар расширен до псевдо-3D + новые
   категории/анимации (`bb0db6d`).
